...
class TestCaseStorage(GeneratorStorage):
    ...
    def get_existing_data_path(self, relative_path: str):
        ...
        self.checker.fail("Expected output {} not found".format(relative_path))
        return None
    ...
    def write_new_data(self, relative_path: str, content: str):
        if REGENERATE_GOLDEN_IMAGES:
            print("RE-GENERATING {!r}".format(relative_path))
        ...
        self.checker.assertEqual(self.get_existing_data(relative_path), content, "Content of {}".format(relative_path))

        # Even if no diff, to be build system friendly, we do NOT expect any
        # actual data writes.
        raise AssertionError("Unexpected write to {}".format(relative_path))
