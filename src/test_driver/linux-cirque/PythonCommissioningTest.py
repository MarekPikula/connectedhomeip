...
class TestCommissioner(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        ...
        self.execute_device_cmd(req_device_id, "pip3 install --break-system-packages {}".format(os.path.join(
            CHIP_REPO, "out/debug/linux_x64_gcc/python/obj/src/controller/python/matter-clusters._build_wheel/matter_clusters-1.0.0-py3-none-any.whl")))
        self.execute_device_cmd(req_device_id, "pip3 install --break-system-packages {}".format(os.path.join(
            CHIP_REPO, "out/debug/linux_x64_gcc/python/obj/src/controller/python/matter-core._build_wheel/matter_core-1.0.0-cp311-abi3-linux_x86_64.whl")))
        self.execute_device_cmd(req_device_id, "pip3 install --break-system-packages {}".format(os.path.join(
            CHIP_REPO, "out/debug/linux_x64_gcc/python/obj/src/controller/python/matter-repl._build_wheel/matter_repl-1.0.0-py3-none-any.whl")))

        command = "gdb -return-child-result -q -ex run -ex bt --args python3 {} -t 150 -d {} --paa-trust-store-path {} --nodeid {}".format(
            os.path.join(
                CHIP_REPO, "src/controller/python/tests/scripts/python_commissioning_flow_test.py"),
            TEST_DISCRIMINATOR,
