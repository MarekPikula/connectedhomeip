...
@click.option("--script", type=click.Path(exists=True),
              default=str(DEFAULT_CHIP_ROOT / 'src/controller/python/tests/scripts/mobile-device-test.py'),
...
def main_impl(app: str, factory_reset: bool, factory_reset_app_only: bool, app_args: str,
    ...
    script_command = [
        script,
        "--fail-on-skipped",
        "--paa-trust-store-path", str(DEFAULT_CHIP_ROOT / MATTER_DEVELOPMENT_PAA_ROOT_CERTS)
