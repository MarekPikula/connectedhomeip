...
from matter.testing.chiptest.concurrency.context import StartStopContextMixin, mp_wrapped_spawn_context
from matter.testing.chiptest.concurrency.process import ProcessConfig, WrappedProcess, with_annotated_exception
from matter.testing.chiptest.concurrency.work_queue import CancellableQueue, QueueCancelled
from matter.testing.chiptest.log_config import LogConfig

if TYPE_CHECKING:
    from matter.testing.chiptest.test_definition import App
...
class XmlRpcServerProcess(WrappedProcess[XmlRpcFuncCall, XmlRpcFuncRet], StartStopContextMixin):
    ...
    def _proc_init(self, exit_stack: contextlib.ExitStack) -> None:
        ...
        server = XmlRpcServerProcess.Server(self._config.stop_timeout_sec)
        for func in APPS_RPC_FUNCS:
            server.register_function(self._create_func(func), self._to_camel_case(func))
