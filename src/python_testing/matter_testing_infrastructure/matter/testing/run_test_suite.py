...
from matter.testing.chiptest import AllChipToolYamlTests, AllDarwinFrameworkToolYamlTests, AllReplYamlTests
from matter.testing.chiptest.accessories import AppsRegister
from matter.testing.chiptest.concurrency.work_queue import CancellableQueue
from matter.testing.chiptest.glob_matcher import GlobMatcher
from matter.testing.chiptest.log_config import LOG_LEVELS, LogConfig, LogMessageCounter
from matter.testing.chiptest.results import ResultError, ResultProcessingThread, RunSummary, TestResult
from matter.testing.chiptest.runner import Executor, Runner
from matter.testing.chiptest.status import PeriodicStatusThread
from matter.testing.chiptest.test_definition import TEST_THREAD_DATASET, SubprocessInfoRepo, TestDefinition, TestRunTime, TestTag
from matter.testing.chiptest.worker import TaskQueueT, WorkerThread
from matter.testing.chipyaml.paths_finder import PathsFinder
from matter.testing.tasks import SubprocessKind

log = logging.getLogger(__name__)

if sys.platform == "linux":
    from matter.testing.chiptest import linux

if sys.platform == 'darwin':
    from matter.testing.chiptest import darwin
...
def main(context: click.Context, log_level: str, log_level_tests: str | None, log_level_rpc: str | None, target: str,
    ...
    match runner:
        case TestRunTime.MATTER_REPL_PYTHON:
            all_tests = list(AllReplYamlTests())
        case TestRunTime.DARWIN_FRAMEWORK_TOOL_PYTHON:
            all_tests = list(AllDarwinFrameworkToolYamlTests())
        case TestRunTime.CHIP_TOOL_PYTHON:
            all_tests = list(AllChipToolYamlTests())
        case _:
            ...
            if sys.platform == 'linux':
                ...
                match commissioning_method:
                    case CommissioningMethod.BLE_WIFI:
                        ...
                        to_terminate.append(linux.WpaSupplicantMock([app_name], "MatterAP", "MatterAPPassword", ns))
